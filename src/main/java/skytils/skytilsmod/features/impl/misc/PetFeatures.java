    ...
    public static class DolphinPetDisplay extends GuiElement {
        ...
        public void render() {
            ...
            if (this.getToggled() && Utils.inSkyblock && player != null && mc.theWorld != null) {
                ...
                Gui.drawModalRectWithCustomSizedTexture((int)x, (int)y, 0, 0, 16, 16, 16, 16);
                List<EntityPlayer> players = mc.theWorld.getPlayers(EntityOtherPlayerMP.class, (p) -> {
                    if (p.getDistanceToEntity(player) > 10 || p.getUniqueID().version() == 2 || p == player) return false;
                    if (!Utils.isInTablist(p)) return false;
                    System.out.println(p.getDisplayName().getFormattedText());
                    return true;
                });
