...
public class DungeonTimer {
  ...
  public void onChat(ClientChatReceivedEvent event) {
    if (!Utils.inDungeons || event.type == 2)
      return;
    ...
    if ((message.equals("§r§aDungeon starts in 1 second.§r") ||
         message.equals("§r§aDungeon starts in 1 second. Get ready!§r")) &&
        dungeonStartTime == -1) {
      dungeonStartTime = System.currentTimeMillis() + 1000;
      return;
    }

    if (message.endsWith("§r§ehas obtained §r§8Wither Key§r§e!§r") ||
        message.equals("§r§eA §r§8Wither Key§r§e was picked up!§r")) {
      witherDoors++;
      return;
    }
    ...
    if (message.equals("§r§cThe §r§c§lBLOOD DOOR§r§c has been opened!§r")) {
      ...
      mc.thePlayer.addChatMessage(new ChatComponentText(
          "§bBlood took " +
          Math.round((bloodOpenTime - dungeonStartTime) / 1000f) +
          " seconds to open."));
      return;
    }

    if (message.equals(
            "§r§c[BOSS] The Watcher§r§f: You have proven yourself. You may pass.§r")) {
      bloodClearTime = System.currentTimeMillis();
      mc.thePlayer.addChatMessage(new ChatComponentText(
          "§bWatcher took " +
          Math.round((bloodClearTime - bloodOpenTime) / 1000f) +
          " seconds to clear."));
      return;
    }

    if ((message.startsWith("§r§c[BOSS] ") ||
         message.startsWith("§r§4[BOSS] ")) &&
        bloodClearTime != -1 && bossEntryTime == -1) {
      bossEntryTime = System.currentTimeMillis();
      return;
    }

    if (message.contains("§r§c☠ §r§eDefeated §r") && bossEntryTime != -1 &&
        bossClearTime == -1) {
      ...
      mc.thePlayer.addChatMessage(new ChatComponentText(
          ...
          + "\n"
          + "§9Boss entry was " +
          timeFormat((double)(bossEntryTime - dungeonStartTime) / 1000f) +
          "."));
      return;
    }

    if (message.startsWith("§r§4[BOSS] Necron") &&
        Objects.equals(DungeonsFeatures.dungeonFloor, "F7")) {
      if (message.endsWith("§r§cFINE! LET'S MOVE TO SOMEWHERE ELSE!!§r") &&
          phase1ClearTime == -1) {
        phase1ClearTime = System.currentTimeMillis();
        mc.thePlayer.addChatMessage(new ChatComponentText(
            "§bPhase 1 took " +
            Math.round((phase1ClearTime - bossEntryTime) / 1000f) +
            " seconds."));
        return;
      }
      if (message.endsWith("§r§cCRAP!! IT BROKE THE FLOOR!§r") &&
          phase2ClearTime == -1) {
        phase2ClearTime = System.currentTimeMillis();
        mc.thePlayer.addChatMessage(new ChatComponentText(
            "§bPhase 2 took " +
            Math.round((phase2ClearTime - phase1ClearTime) / 1000f) +
            " seconds."));
        return;
      }
      if (message.endsWith("§r§cTHAT'S IT YOU HAVE DONE IT!§r") &&
          phase3ClearTime == -1) {
        phase3ClearTime = System.currentTimeMillis();
        mc.thePlayer.addChatMessage(new ChatComponentText(
            "§bPhase 3 took " +
            Math.round((phase3ClearTime - phase2ClearTime) / 1000f) +
            " seconds."));
        return;
      }
    }
  ...
  private static String timeFormat(double seconds) {
    if (seconds >= 60) {
      return MathUtil.fastFloor(seconds / 60) + "m " +
  ...
  public static class DungeonTimerElement extends GuiElement {
    ...
    public void render() {
      ...
      if (this.getToggled() && Utils.inDungeons && player != null &&
          world != null) {
        ...
        String displayText =
            ...
            (dungeonStartTime == -1
                 ? "0s"
                 : timeFormat((double)((bossClearTime == -1
                                            ? (scoreShownAt == -1
                                                   ? System.currentTimeMillis()
                                                   : scoreShownAt)
                                            : bossClearTime) -
            ...
            (bloodOpenTime == -1
                 ? "0s"
                 : timeFormat((double)(bloodOpenTime - dungeonStartTime) /
                              1000f)) +
            "\n\u00a7cWatcher Clear: " +
            (bloodClearTime == -1
                 ? "0s"
                 : timeFormat((double)(bloodClearTime - bloodOpenTime) /
                              1000f)) +
            "\n\u00a79Boss Entry: " +
            (bossEntryTime == -1
                 ? "0s"
                 : timeFormat((double)(bossEntryTime - dungeonStartTime) /
                              1000f)) +
            "\n\u00a7bBoss Clear: " +
            (bossClearTime == -1
                 ? "0s"
                 : timeFormat((double)(bossClearTime - bossEntryTime) / 1000f));
        String[] lines = displayText.split("\n");
        for (int i = 0; i < lines.length; i++) {
          SmartFontRenderer.TextAlignment alignment =
              leftAlign ? SmartFontRenderer.TextAlignment.LEFT_RIGHT
                        : SmartFontRenderer.TextAlignment.RIGHT_LEFT;
          ScreenRenderer.fontRenderer.drawString(
              lines[i],
              leftAlign ? this.getActualX() : this.getActualX() + getWidth(),
              this.getActualY() + i * ScreenRenderer.fontRenderer.FONT_HEIGHT,
              CommonColors.WHITE, alignment,
    ...
    @Override
    public int getWidth() {
      return ScreenRenderer.fontRenderer.getStringWidth(
          "\u00a7cWatcher Clear: 0s");
    }
    ...
    public void render() {
      ...
      if (this.getToggled() && Utils.inDungeons && player != null &&
        ...
        String displayText =
            "\u00a7bPhase 1: " +
            timeFormat(
                (double)((phase1ClearTime == -1
                              ? (scoreShownAt == -1 ? System.currentTimeMillis()
                                                    : scoreShownAt)
                              : phase1ClearTime) -
                         bossEntryTime) /
                1000f) +
            ...
            (phase1ClearTime == -1
                 ? "0s"
                 : timeFormat((double)((phase2ClearTime == -1
                                            ? (scoreShownAt == -1
                                                   ? System.currentTimeMillis()
                                                   : scoreShownAt)
                                            : phase2ClearTime) -
            ...
            (phase2ClearTime == -1
                 ? "0s"
                 : timeFormat((double)((phase3ClearTime == -1
                                            ? (scoreShownAt == -1
                                                   ? System.currentTimeMillis()
                                                   : scoreShownAt)
                                            : phase3ClearTime) -
