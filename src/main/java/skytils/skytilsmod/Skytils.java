...
@Mod(modid = Skytils.MODID, name = Skytils.MOD_NAME, version = Skytils.VERSION,
...
public class Skytils {
  ...
  public void postInit(FMLPostInitializationEvent event) {
    ...
    if (!cch.getCommands().containsKey("rp")) {
      ...
      ((AccessorCommandHandler)cch)
          .getCommandMap()
          .put("rp", new RepartyCommand());
    }
    if (Skytils.config.overrideReparty) {
      ...
      for (Map.Entry<String, ICommand> entry : cch.getCommands().entrySet()) {
        if (Objects.equals(entry.getKey(), "reparty") ||
            Objects.equals(entry.getKey(), "rp")) {
  ...
  public void onTick(TickEvent.ClientTickEvent event) {
    ...
    if (mc.thePlayer != null && sendMessageQueue.size() > 0 &&
        System.currentTimeMillis() - lastChatMessage > 200) {
