...
public class EssentialPlatformSetup {
    private static final String[] dataURLCandidates = {System.getProperty("sharttils.dataURL"), Reference.dataUrl, "https://sharttilsmod-data.pages.dev/", "https://cdn.jsdelivr.net/gh/Sharttils/SharttilsMod-Data@main/"};
    private static final boolean isDev = System.getProperty("sharttils.testEssentialSetup") != null;
    ...
    private static String requestEssentialResource(String url) throws IOException {
        ...
        conn.addRequestProperty("User-Agent", "Sharttils/" + Sharttils.VERSION);
    ...
    public static void setup() {
        try {
            ...
            me.load(EssentialPlatformSetup.class.getResourceAsStream("sharttilsletsencrypt.jks"), "sharttilsontop".toCharArray());
        ...
        try {
            ...
            if (Launch.classLoader.getClassBytes("net.minecraft.world.World") != null || essentialLoc.isDirectory() || Sharttils.VERSION.endsWith("-dev"))
                return;
            ...
            if (!essentialDownloads.has(Sharttils.VERSION)) loadEssential();
            if (!essentialDownloads.get(Sharttils.VERSION).getAsString().equalsIgnoreCase(essentialVersion))
                loadEssential();
        } catch (IOException e) {
