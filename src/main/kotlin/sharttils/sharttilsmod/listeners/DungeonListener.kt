...
import sharttils.sharttilsmod.features.impl.dungeons.ScoreCalculation
import sharttils.sharttilsmod.features.impl.handlers.CooldownTracker
...
object DungeonListener {
    ...
    fun onPacket(event: MainReceivePacketEvent<*, *>) {
        ...
        if (event.packet is S02PacketChat) {
            if (event.packet.chatComponent.unformattedText.startsWith("Dungeon starts in 1 second.")) {
                ...
                if (Sharttils.config.dungeonDeathCounter) {
                    TickTask(6) {
                ...
                if (Sharttils.config.autoRepartyOnDungeonEnd) {
    ...
    private fun getMembers() {
        ...
        if (partyCount != team.size) {
            UChat.chat("§9§lSharttils §8» §cSomething isn't right! I expected $partyCount members but only got ${team.size}")
        }
        if (team.values.any { it.dungeonClass == DungeonClass.EMPTY }) {
            UChat.chat("§9§lSharttils §8» §cSomething isn't right! One or more of your party members has an empty class! Could the server be lagging?")
        }
    ...
    fun checkSpiritPet() {
        if (Sharttils.hylinAPI.key.isNotEmpty()) {
            Sharttils.threadPool.submit {
                runCatching {
                    for (teammate in team.values) {
                        val name = teammate.playerName
                        if (hutaoFans.getIfPresent(name) != null) continue
                        val uuid = teammate.player?.uniqueID ?: Sharttils.hylinAPI.getUUIDSync(
                            name
                        )
                        val profile = Sharttils.hylinAPI.getLatestSkyblockProfileForMemberSync(
                            uuid
                        ) ?: continue
