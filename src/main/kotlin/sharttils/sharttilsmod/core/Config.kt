...
import sharttils.sharttilsmod.gui.SpiritLeapNamesGui
...
object Config : Vigilant(
    File("./config/sharttils/config.toml"),
    if (Utils.isBSMod) "BSMod" else "Sharttils",
    sortingBehavior = ConfigSorting
) {
    @Property(
        type = PropertyType.SWITCH, name = "Fetch Lowest BIN Prices",
        description = "Fetches the lowest BIN features for Sharttils to use.\nSome features will be hidden and will not work if this switch isn't on.",
    ...
    @Property(
        type = PropertyType.BUTTON, name = "Join the Sharttils Discord",
        description = "Join the Sharttils Discord server for help using any of the features.",
        category = "General", subcategory = "Other",
        placeholder = "Join"
    )
    @Suppress("unused")
    fun openDiscordLink() {
        UDesktop.browse(URI.create("https://discord.gg/sharttils"))
    }

    @Property(
        type = PropertyType.SWITCH, name = "First Launch",
        description = "Used to see if the user is a new user of Sharttils.",
        category = "General", subcategory = "Other",
        hidden = true
    )
    ...
    @Property(
        type = PropertyType.TEXT, name = "Last Launched Sharttils Version",
        category = "General", subcategory = "Other",
        hidden = true
    )
    ...
    @Property(
        ...
        description = "Adds a button to configure Sharttils to the pause menu.",
        category = "General", subcategory = "Other"
    )
    ...
    @Property(
        ...
        description = "Sets the menu to the Sharttils options menu instead of exiting when on a Sharttils config menu.",
        category = "General", subcategory = "Other"
    )
    ...
    @Property(
        ...
        description = "Uses Sharttils' reparty command instead of other mods'. \n§cRequires restart to disable",
    ...
    @Property(
        ...
        category = "Dungeons", subcategory = "Score Calculation",
        placeholder = "Sharttils > 270 score"
    )
    var message270Score = ""
    ...
    @Property(
        ...
        category = "Dungeons", subcategory = "Score Calculation",
        placeholder = "Sharttils > 300 score"
    )
    var message300Score = ""
    ...
    @Property(
        ...
        description = "Replace Sadan's interest display with Sharttils' own.",
        category = "Dungeons", subcategory = "Quality of Life"
    )
    ...
    fun spiritLeapNameButton() {
        Sharttils.displayScreen = SpiritLeapNamesGui()
    }
    ...
    @Property(
        ...
        description = "Displays Sharttils' own auction input GUI instead of a sign.",
        category = "Miscellaneous", subcategory = "Quality of Life"
    )
    ...
    @Property(
        ...
        description = "Allows you to move the action bar as if it were a Sharttils HUD element.",
        category = "Miscellaneous", subcategory = "Quality of Life"
    )
    ...
    @Property(
        ...
        description = "Allows you to move the item name highlight as if it were a Sharttils HUD element.",
        category = "Miscellaneous", subcategory = "Quality of Life"
    )
    ...
    init {
        ...
        registerListener("itemRarityShape") { i: Int ->
            if (i == 4 && Loader.instance().hasReachedState(LoaderState.POSTINITIALIZATION)) {
                ...
                runCatching {
                    val loc = ResourceLocation("sharttils:gui/customrarity.png")
        ...
        registerListener("apiKey") { key: String ->
            Sharttils.hylinAPI.key = key
        }
    }

    fun init() {
        initialize()
        if (Sharttils.config.lastLaunchedVersion != Sharttils.VERSION) {
            val ver = UpdateChecker.SharttilsVersion(Sharttils.config.lastLaunchedVersion)
            when {
                !ver.isSafe || ver < UpdateChecker.SharttilsVersion("1.2-pre3") || Sharttils.config.lastLaunchedVersion == "0" -> {
        ...
        lastLaunchedVersion = Sharttils.VERSION
        markDirty()
    }
