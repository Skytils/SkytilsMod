...
class MayorDiana {
    ...
    fun onPacket(event: PacketEvent.ReceiveEvent) {
        ...
        if (Sharttils.config.trackGaiaHits && event.packet is S29PacketSoundEffect) {
    ...
    fun onCheckRender(event: CheckRenderEntityEvent<*>) {
        ...
        if (Sharttils.config.removeLeftOverBleeds && mc.theWorld != null && entity is EntityArmorStand && entity.hasCustomName() && entity.displayName.formattedText.startsWith(
                "§c☣ §fBleeds: §c"
    ...
    fun onTick(event: TickEvent.ClientTickEvent) {
        if (!Utils.inSkyblock) return
        if (event.phase == TickEvent.Phase.START && Sharttils.config.trackGaiaHits) for (golem in gaiaConstructHits.keys) {
