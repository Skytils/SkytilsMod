...
/**
 * Original code was taken from Danker's Skyblock Mod under GPL 3.0 license and modified by the Sharttils team
...
class ThreeWeirdosSolver {
    ...
    fun onChat(event: ClientChatReceivedEvent) {
        if (!Sharttils.config.threeWeirdosSolver || !Utils.inDungeons) return
        ...
        if (unformatted.contains("[NPC]")) {
            if (solutions.size == 0) {
                UChat.chat("§cSharttils failed to load solutions for Three Weirdos.")
    ...
    fun onPlayerInteract(event: PlayerInteractEvent) {
        if (!Utils.inDungeons || !Sharttils.config.threeWeirdosSolver || event.action != PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) return
    ...
    fun onWorldRender(event: RenderWorldLastEvent) {
        if (!Sharttils.config.threeWeirdosSolver || !Utils.inDungeons || riddleNPC == null) return
        if (riddleChest == null) {
