...
object ShootTheTargetSolver {
    ...
    fun onBlockChange(event: BlockChangeEvent) {
        if (!Utils.inDungeons || DungeonTimer.phase2ClearTime == -1L || !Sharttils.config.shootTheTargetSolver) return
        val pos = event.pos
        val old = event.old
    ...
    fun onRenderWorld(event: RenderWorldLastEvent) {
        if (!Sharttils.config.shootTheTargetSolver || shot.isEmpty()) return
