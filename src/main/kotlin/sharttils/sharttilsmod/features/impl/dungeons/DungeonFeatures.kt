...
import sharttils.sharttilsmod.mixins.transformers.accessors.AccessorEnumDyeColor
...
object DungeonFeatures {
    ...
    fun onBlockChange(event: BlockChangeEvent) {
        if (hasBossSpawned && Sharttils.config.spiritBearTimer && dungeonFloor?.endsWith('4') == true) {
    ...
    fun onTick(event: ClientTickEvent) {
        ...
        if (Utils.inDungeons) {
            ...
            if (isInTerracottaPhase && terracottaEndTime > 0 && Sharttils.config.showSadanInterest) {
            ...
            if (Sharttils.config.spiritPetWarning && !alertedSpiritPet && DungeonTimer.dungeonStartTime == -1L && mc.theWorld.loadedEntityList.any {
            ...
            if (Sharttils.config.findCorrectLivid && !foundLivid) {
                if (equalsOneOf(dungeonFloor, "F5", "M5")) {
                    when (Sharttils.config.lividFinderType) {
                        1 -> {
                        ...
                        0 -> {
                            if (hasBossSpawned && mc.thePlayer.isPotionActive(Potion.blindness) && (lividJob == null || lividJob?.isCancelled == true || lividJob?.isDone == true)) {
                                lividJob = Sharttils.threadPool.submit {
                                    ...
                                    for (entity in mc.theWorld.loadedEntityList) {
                                        ...
                                        if ((a != null && entity.name.startsWith("$a﴾ $a§lLivid")) || fallBackColor) {
                                            if (fallBackColor && !(a != null && entity.name.startsWith("$a﴾ $a§lLivid"))) {
                                                UChat.chat("§bBlock color ${color.name} should be mapped to ${otherColor}${otherColor.name}§b. Please report this to discord.gg/sharttils")
                                            }
                                            lividTag = entity
                                            val aabb = AxisAlignedBB(
                                                lividTag!!.posX - 0.5,
    ...
    fun onBossBarSet(event: BossBarEvent.Set) {
        ...
        if (equalsOneOf(dungeonFloor, "F7", "M7")) {
            if (equalsOneOf(unformatted, "Maxor", "Storm", "Goldor", "Necron")) {
                when (Sharttils.config.necronHealth) {
                    2 -> {
        ...
        if (equalsOneOf(dungeonFloor, "F6", "M6")) {
            ...
            } else if (terracottaEndTime > 0 && Sharttils.config.showSadanInterest) {
                event.isCanceled = true
            }
            return
        }
    ...
    fun onDeath(event: LivingDeathEvent) {
        ...
        if (event.entity is EntityBlaze && ++blazes == 10 && Sharttils.config.sayBlazeDone) {
            Sharttils.sendMessageQueue.add("/pc Blaze Done")
        }
    }
    ...
    fun onChat(event: ClientChatReceivedEvent) {
        ...
        if (Utils.inDungeons) {
            if (Sharttils.config.autoCopyFailToClipboard) {
                ...
                if (deathFailMatcher.find() || (unformatted.startsWith("[CROWD]") && thornMissMessages.any {
                    ...
                    if (!unformatted.contains("disconnect")) {
                        ...
                        UChat.chat("§9§lSharttils §8» §aCopied fail to clipboard.")
                    }
            ...
            if (Sharttils.config.hideF4Spam && unformatted.startsWith("[CROWD]") && thornMissMessages.none {
                    unformatted.contains(
                        it,
                        true
                    )
    ...
    fun onSendChatMessage(event: SendChatMessageEvent) {
        if (event.message.startsWith("/sharttilscopy") && !event.addToChat) {
            UChat.chat("§9§lSharttils §8» §aCopied to clipboard.")
            GuiScreen.setClipboardString(event.message.substring("/sharttilscopy ".length))
            event.isCanceled = true
        }
    }
    ...
    fun onCheckRender(event: CheckRenderEntityEvent<*>) {
        if (!Utils.inDungeons) return
        if (Sharttils.config.hideArcherBonePassive && event.entity is EntityItem && event.entity.entityItem.itemDamage == 15 && event.entity.entityItem.item === Items.dye)
            event.isCanceled = true
    }
    ...
    fun onRenderLivingPre(event: RenderLivingEvent.Pre<*>) {
        if (Utils.inDungeons) {
            if (Sharttils.config.boxSpiritBow && equalsOneOf(
                    dungeonFloor,
                    "F4",
                    "M4"
            ...
            if (event.entity is EntityArmorStand && event.entity.hasCustomName()) {
                if (Sharttils.config.hideWitherMinerNametags) {
                ...
                if (Sharttils.config.hideF4Nametags) {
                    val name = event.entity.customNameTag.stripControlCodes()
                    if (name.contains("Spirit") && !name.contains("Spirit Bear")) {
                        mc.theWorld.removeEntity(event.entity)
                    }
                }
                if (Sharttils.config.hideTerracotaNametags) {
                    val name = event.entity.customNameTag.stripControlCodes()
                    if (name.contains("Terracotta ")) mc.theWorld.removeEntity(event.entity)
                }
                if (Sharttils.config.hideNonStarredNametags) {
                    val name = event.entity.customNameTag.stripControlCodes()
                ...
                if (Sharttils.config.hideFairies && event.entity.heldItem != null && ItemUtil.getSkullTexture(event.entity.heldItem) == "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvOTZjM2UzMWNmYzY2NzMzMjc1YzQyZmNmYjVkOWE0NDM0MmQ2NDNiNTVjZDE0YzljNzdkMjczYTIzNTIifX19") {
                    event.isCanceled = true
                }
            }
            if (!mc.renderManager.isDebugBoundingBox && !event.entity.isInvisible) {
                if (event.entity is EntityBat && Sharttils.config.showBatHitboxes && !hasBossSpawned &&
                ...
                if (event.entity is EntitySkeleton && Sharttils.config.boxSkeletonMasters && ItemUtil.getSkyBlockItemID(
                ...
                if (hasBossSpawned && Sharttils.config.boxSpiritBears && event.entity.name == "Spirit Bear" && event.entity is EntityOtherPlayerMP) {
                    RenderUtil.drawOutlinedBoundingBox(
                        event.entity.entityBoundingBox,
    ...
    fun onReceivePacket(event: MainReceivePacketEvent<*, *>) {
        ...
        if (event.packet is S45PacketTitle) {
            ...
            if (packet.message != null && mc.thePlayer != null) {
                ...
                if (Sharttils.config.hideTerminalCompletionTitles && Utils.inDungeons && !unformatted.contains(mc.thePlayer.name) && (unformatted.contains(
        ...
        if (event.packet is S29PacketSoundEffect) {
            val packet = event.packet
            if (Sharttils.config.disableTerracottaSounds && isInTerracottaPhase) {
        ...
        if (event.container is ContainerChest) {
            ...
            when {
                chestName.endsWith(" Chest") -> {
                    if (Sharttils.config.kismetRerollConfirm > 0 && event.slotId == 50) {
                        rerollClicks++
                        val neededClicks = Sharttils.config.kismetRerollConfirm - rerollClicks
                        if (neededClicks > 0) {
                            event.isCanceled = true
                        }
                    }
                ...
                chestName == "Start Dungeon?" -> {
                    if (!startWithoutFullParty && Sharttils.config.noChildLeftBehind && event.slot?.stack?.displayName == "§aStart Dungeon?") {
                        val teamCount =
    ...
    fun onTooltip(event: ItemTooltipEvent) {
        if (event.itemStack != null) {
            if (Utils.inDungeons && Sharttils.config.kismetRerollConfirm > 0 && ItemUtil.getDisplayName(event.itemStack)
    ...
    class SpiritBearSpawnTimer : GuiElement("Spirit Bear Spawn Timer", FloatPair(0.05f, 0.4f)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.spiritBearTimer
    ...
    internal class LividGuiElement : GuiElement("Livid HP", FloatPair(0.05f, 0.4f)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.findCorrectLivid
