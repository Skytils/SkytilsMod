...
import sharttils.sharttilsmod.mixins.transformers.accessors.AccessorChatComponentText
...
object ScoreCalculation {
        ...
        ((skillScore.zip(discoveryScore)).zip(speedScore.zip(bonusScore))).map { (first, second) ->
            ...
            state.onSetValue { score ->
                ...
                if (!hasSaid270 && score >= 270) {
                    hasSaid270 = true
                    if (Sharttils.config.createTitleOn270Score) GuiManager.createTitle(
                        "§c§l" + Sharttils.config.messageTitle270Score.ifBlank { "270" },
                        20
                    )
                    if (Sharttils.config.sendMessageOn270Score) Sharttils.sendMessageQueue.add("/pc Sharttils-SC > ${Sharttils.config.message270Score.ifBlank { "270 score" }}")
                }
                if (!hasSaid300 && score >= 300) {
                    hasSaid300 = true
                    if (Sharttils.config.createTitleOn300Score) GuiManager.createTitle(
                        "§c§l" + Sharttils.config.messageTitle300Score.ifBlank { "300" },
                        20
                    )
                    if (Sharttils.config.sendMessageOn300Score) Sharttils.sendMessageQueue.add("/pc Sharttils-SC > ${Sharttils.config.message300Score.ifBlank { "300 score" }}")
                }
            }
        }
    ...
    fun updateText(score: Int) {
        Utils.checkThreadAndQueue {
            ...
            if (Sharttils.config.minimizedScoreCalculation) {
                val color = when {
                    score < 270 -> 'c'
                    score < 300 -> 'e'
                    else -> 'a'
    ...
    fun onTick(event: ClientTickEvent) {
        ...
        if (mc.thePlayer != null && mc.theWorld != null && Utils.inDungeons) {
            if (Sharttils.config.showScoreCalculation && ticks % 5 == 0) {
                isPaul.set(
    ...
    fun onChatReceived(event: ClientChatReceivedEvent) {
        ...
        if (Sharttils.config.scoreCalculationReceiveAssist) {
            if (unformatted.startsWith("Party > ") || (unformatted.contains(":") && !unformatted.contains(">"))) {
                if (unformatted.contains("\$SHARTTILS-DUNGEON-SCORE-MIMIC$") || (Sharttils.config.receiveHelpFromOtherModMimicDead && unformatted.containsAny(
                ...
                if (unformatted.contains("\$SHARTTILS-DUNGEON-SCORE-ROOM$")) {
    ...
    fun canYouPleaseStopCryingThanks(event: ClientChatReceivedEvent) {
        ...
        val unformatted = event.message.unformattedText.stripControlCodes()
        if ((unformatted.startsWith("Party > ") || unformatted.startsWith("P > ")) && unformatted.contains(": Sharttils-SC > ")) {
            event.message.siblings.filterIsInstance<ChatComponentText>().forEach {
                it as AccessorChatComponentText
                if (it.text.startsWith("Sharttils-SC > ")) {
                    it.text = it.text.substringAfter("Sharttils-SC > ")
                } else if (it.text.startsWith("\$SHARTTILS-DUNGEON-SCORE-MIMIC\$")) {
                    it.text = it.text.replace("\$SHARTTILS-DUNGEON-SCORE-MIMIC\$", "Mimic Killed!")
                }
            }
        }
    }
    ...
    fun onEntityDeath(event: LivingDeathEvent) {
        ...
        if (event.entity is EntityZombie) {
            ...
            if (entity.isChild && entity.getCurrentArmor(0) == null && entity.getCurrentArmor(1) == null && entity.getCurrentArmor(
                ...
                if (!mimicKilled.get()) {
                    mimicKilled.set(true)
                    if (Sharttils.config.scoreCalculationAssist) {
                        Sharttils.sendMessageQueue.add("/pc \$SHARTTILS-DUNGEON-SCORE-MIMIC$")
                    }
                }
            }
        }
    ...
    class HugeCryptsCounter : GuiElement("Dungeon Crypts Counter", 2f, FloatPair(200, 200)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.bigCryptsCounter
    ...
    class ScoreCalculationElement : GuiElement("Dungeon Score Estimate", FloatPair(200, 100)) {
        ...
        override val height: Int
            get() = if (Sharttils.config.minimizedScoreCalculation) ScreenRenderer.fontRenderer.FONT_HEIGHT else ScreenRenderer.fontRenderer.FONT_HEIGHT * demoText.size
        override val width: Int
            get() = demoText.maxOf { ScreenRenderer.fontRenderer.getStringWidth(it) }

        override val toggled: Boolean
            get() = Sharttils.config.showScoreCalculation
