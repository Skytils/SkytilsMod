...
import sharttils.sharttilsmod.mixins.transformers.accessors.AccessorModelDragon
...
object MasterMode7Features {
    ...
    fun onBlockChange(event: BlockChangeEvent) {
        ...
        if (Sharttils.config.witherKingDragonSlashAlert) {
    ...
    fun onPacket(event: MainReceivePacketEvent<*, *>) {
        ...
        if (event.packet is S2CPacketSpawnGlobalEntity && event.packet.func_149053_g() == 1) {
            ...
            if (spawningDragons.add(drag)) {
                ...
                if (Sharttils.config.witherKingDragonSpawnAlert) UChat.chat("§c§lThe ${drag.chatColor}§l${drag.name} §c§lis spawning! §f(${x}, ${y}, ${z})")
            }
        }
    }
    ...
    fun onMobSpawned(entity: Entity) {
        if (DungeonTimer.phase4ClearTime != -1L && entity is EntityDragon) {
            ...
            (entity as ExtensionEntityLivingBase).sharttilsHook.colorMultiplier = type.color
            (entity as ExtensionEntityLivingBase).sharttilsHook.masterDragonType = type
    ...
    fun onDeath(event: LivingDeathEvent) {
        if (event.entity is EntityDragon) {
            val item = (event.entity as ExtensionEntityLivingBase).sharttilsHook.masterDragonType ?: return
    ...
    fun onRenderLivingPost(event: RenderLivingEvent.Post<*>) {
        ...
        if (Sharttils.config.showWitherKingDragonsHP && DungeonTimer.phase4ClearTime != -1L && entity is EntityDragon) {
            ...
            RenderUtil.drawLabel(
                ...
                "${NumberUtil.format(event.entity.health)} ${
                    "§fR".toStringIfTrue(
                        entity.sharttilsHook.masterDragonType?.bb?.isVecInside(entity.positionVector)
                    )
                }",
                color,
    ...
    fun onRenderWorld(event: RenderWorldLastEvent) {
        if (Sharttils.config.showWitherKingStatueBox && DungeonTimer.phase4ClearTime != -1L) {
    ...
    fun getHurtOpacity(
        ...
        if (!Sharttils.config.changeHurtColorOnWitherKingsDragons) return value
        lastDragon as ExtensionEntityLivingBase
        return if (lastDragon.sharttilsHook.colorMultiplier != null) {
    ...
    fun getEntityTexture(entity: EntityDragon, cir: CallbackInfoReturnable<ResourceLocation>) {
        if (!Sharttils.config.retextureWitherKingsDragons) return
        entity as ExtensionEntityLivingBase
        val type = entity.sharttilsHook.masterDragonType ?: return
    ...
    fun shouldHideDragonDeath() =
        Utils.inDungeons && DungeonTimer.phase4ClearTime != -1L && Sharttils.config.hideWitherKingDragonDeath
}
...
enum class WitherKingDragons(
    ...
    val texture = ResourceLocation("sharttils", "textures/dungeons/m7/dragon_${this.name.lowercase()}.png")
    private val a = 13.5
    val bb = blockPos.run {
