...
import sharttils.sharttilsmod.utils.tictactoe.AlphaBetaAdvanced
import sharttils.sharttilsmod.utils.tictactoe.Board
...
class TicTacToeSolver {
    ...
    fun onTick(event: TickEvent.ClientTickEvent) {
        if (!Utils.inDungeons || !Sharttils.config.ticTacToeSolver) return
    ...
    fun onRenderWorld(event: RenderWorldLastEvent) {
        ...
        if (bestMove != null) {
            RenderUtil.drawOutlinedBoundingBox(
                ...
                Sharttils.config.ticTacToeSolverColor,
                3f,
                event.partialTicks
            )
        }
