...
object ChangeAllToSameColorSolver {
    ...
    fun onBackgroundDrawn(event: GuiContainerEvent.BackgroundDrawnEvent) {
        if (!Utils.inDungeons || !Sharttils.config.changeAllSameColorTerminalSolver || event.container !is ContainerChest || event.chestName != "Change all to same color!") return
        ...
        for ((slot, clicks) in mapping) {
            ...
            if (Sharttils.config.changeToSameColorMode == 1) {
    ...
    fun onTooltip(event: ItemTooltipEvent) {
        ...
        if (!Sharttils.config.changeAllSameColorTerminalSolver) return
