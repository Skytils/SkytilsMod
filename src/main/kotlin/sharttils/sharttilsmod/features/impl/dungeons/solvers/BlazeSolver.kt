...
class BlazeSolver {
    ...
    fun onTick(event: ClientTickEvent) {
        ...
        if (ticks % 4 == 0) {
            if (Sharttils.config.blazeSolver) {
                orderedBlazes.clear()
                ...
                orderedBlazes.sortWith { blaze1, blaze2 ->
                    ...
                    if (compare == 0 && !impossible) {
                        impossible = true
                        UChat.chat("§c[§f§lWARNING§c] Sharttils detected two blazes with the exact same amount of health!")
    ...
    fun onWorldRender(event: RenderWorldLastEvent) {
        if (Sharttils.config.blazeSolver && Utils.inDungeons && orderedBlazes.size > 0) {
            if (blazeMode < 0) {
                ...
                RenderUtil.drawLabel(
                    ...
                    "§lSmallest",
                    Sharttils.config.lowestBlazeColor,
                    event.partialTicks
                )
            }
            if (blazeMode > 0) {
                ...
                RenderUtil.drawLabel(
                    ...
                    "§lBiggest",
                    Sharttils.config.highestBlazeColor,
                    event.partialTicks
                )
            }
        }
