...
object ClickInOrderSolver {
    ...
    fun onBackgroundDrawn(event: GuiContainerEvent.BackgroundDrawnEvent) {
        if (!Utils.inDungeons || !Sharttils.config.clickInOrderTerminalSolver || event.container !is ContainerChest) return
        ...
        if (firstSlot != null) {
            ...
            if (slot != null) slot highlight Sharttils.config.clickInOrderFirst
        }
        if (secondSlot != null) {
            val slot = invSlots[secondSlot]
            if (slot != null) slot highlight Sharttils.config.clickInOrderSecond
        }
        if (thirdSlot != null) {
            val slot = invSlots[thirdSlot]
            if (slot != null) slot highlight Sharttils.config.clickInOrderThird
        }
    ...
    fun onDrawSlotLow(event: GuiContainerEvent.DrawSlotEvent.Pre) {
        ...
        if (!Sharttils.config.clickInOrderTerminalSolver) return
