...
import sharttils.hylin.skyblock.Pet
...
import sharttils.sharttilsmod.utils.SkillUtils.level
import java.util.*
...
object PartyFinderStats {
    ...
    fun onChat(event: ClientChatReceivedEvent) {
        ...
        if (Sharttils.config.partyFinderStats) {
            ...
            Sharttils.launch {
                Sharttils.hylinAPI.getUUID(username).whenComplete { uuid ->
                    Sharttils.hylinAPI.getLatestSkyblockProfileForMember(uuid).whenComplete { profile ->
                        profile?.run { playerStats(username, uuid, this) }
                    }.catch { e ->
                        UChat.chat("§cUnable to retrieve profile information: ${e.message?.replace(Sharttils.config.apiKey, "*".repeat(Sharttils.config.apiKey.length))}")
                    }
                }.catch { e ->
    ...
    private fun playerStats(username: String, uuid: UUID, profileData: Member) {
        Sharttils.hylinAPI.getPlayer(uuid).whenComplete { playerResponse ->
            try {
                    ...
                    val component = UMessage("&2&l-----------------------------\n")
                        ...
                        .append(
                            UTextComponent("§7[Stats]\n\n")
                                .setHoverText("§7Click to run: /sharttilscata $username")
                                .setClick(
                                    ClickEvent.Action.RUN_COMMAND, "/sharttilscata $username"
                                )
                        )
            ...
            UChat.chat("§cFailed to get dungeon stats: ${e.message?.replace(Sharttils.config.apiKey, "*".repeat(Sharttils.config.apiKey.length))}")
        }
    }
