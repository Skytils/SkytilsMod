...
class WaterBoardSolver {
    ...
    fun onTick(event: ClientTickEvent) {
        ...
        if (!Sharttils.config.waterBoardSolver) return
        ...
        if (ticks % 20 == 0) {
            if (DungeonListener.missingPuzzles.contains("Water Board") && variant == -1 && (job == null || job?.isCancelled == true || job?.isDone == true)) {
                job = Sharttils.threadPool.submit {
    ...
    fun onRenderWorld(event: RenderWorldLastEvent) {
        if (!Sharttils.config.waterBoardSolver || !DungeonListener.missingPuzzles.contains("Water Board")) return
