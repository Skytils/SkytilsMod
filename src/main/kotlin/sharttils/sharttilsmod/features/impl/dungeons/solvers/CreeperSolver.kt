...
class CreeperSolver {
    ...
    fun onTick(event: ClientTickEvent) {
        ...
        if (ticks % 20 == 0) {
            if (Sharttils.config.creeperBeamsSolver && Utils.inDungeons && world != null && player != null && DungeonListener.missingPuzzles.contains(
                    "Creeper Beams"
                )
            ) {
                val x = player.posX
    ...
    fun onWorldRender(event: RenderWorldLastEvent) {
        if (Sharttils.config.creeperBeamsSolver && solutionPairs.isNotEmpty() && !creeper!!.isDead && DungeonListener.missingPuzzles.contains(
                "Creeper Beams"
            )
        ) {
