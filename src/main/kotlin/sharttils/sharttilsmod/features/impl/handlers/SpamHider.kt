...
import sharttils.sharttilsmod.core.GuiManager
...
import sharttils.sharttilsmod.utils.toasts.*
import sharttils.sharttilsmod.utils.toasts.BlessingToast.BlessingBuff
import java.io.*
import java.util.*
...
class SpamHider : PersistentSave(File(Sharttils.modDir, "spamhider.json")) {
    ...
    fun onActionBarDisplay(event: SetActionBarEvent) {
        ...
        if (Sharttils.config.manaUseHider != 0 && manaUsageMatcher.find()) {
            ...
            if (Sharttils.config.manaUseHider == 2) {
    ...
    fun onChatPacket(event: ReceiveEvent) {
        ...
        if (Utils.inSkyblock && packet is S0BPacketAnimation && packet.animationType == 0) {
            val entity = Sharttils.mc.theWorld.getEntityByID(packet.entityID) ?: return
            if (entity !is EntityOtherPlayerMP) return
            if (entity.heldItem?.item != deadBush || entity.getDistanceSqToEntity(Sharttils.mc.thePlayer) > 4 * 4) return
        ...
        if (formatted.startsWith("§aYou are playing on profile:")) {
            when (Sharttils.config.profileHider) {
                1, 2 -> cancelChatPacket(event, Sharttils.config.profileHider == 2)
            }
        }
        ...
        try {
            if (Utils.inDungeons) {
                ...
                when {
                    unformatted.startsWith("[NPC] Mort") -> {
                        when (Sharttils.config.hideMortMessages) {
                            1, 2 -> cancelChatPacket(event, Sharttils.config.hideMortMessages == 2)
                        }
                    }
                    unformatted.startsWith("[BOSS]") && !unformatted.startsWith("[BOSS] The Watcher") -> {
                        when (Sharttils.config.hideBossMessages) {
                            1, 2 -> cancelChatPacket(event, Sharttils.config.hideBossMessages == 2)
                        }
                    }
                    Sharttils.config.hideOruoMessages && unformatted.startsWith("[STATUE] Oruo the Omniscient: ") && !unformatted.contains(
            ...
            when {
                ...
                unformatted.startsWith("Autopet equipped your") -> {
                    when (Sharttils.config.hideAutopetMessages) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.hideAutopetMessages == 2)
                    }
                }
                // CantUseAbilityHider
                unformatted.startsWith("You cannot use abilities in this room!") -> {
                    when (Sharttils.config.hideCantUseAbility) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.hideCantUseAbility == 2)
                    }
                }
                //No enemies nearby
                formatted.startsWith("§r§cThere are no enemies nearby!") -> {
                    when (Sharttils.config.hideNoEnemiesNearby) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.hideNoEnemiesNearby == 2)
                    }
                }

                // Implosion
                formatted.contains("§r§7Your Implosion hit ") -> {
                    when (Sharttils.config.implosionHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.implosionHider == 2)
                    }
                }

                // Midas Staff
                formatted.contains("§r§7Your Molten Wave hit ") -> {
                    when (Sharttils.config.midasStaffHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.midasStaffHider == 2)
                    }
                }

                // Spirit Sceptre
                formatted.contains("§r§7Your Spirit Sceptre hit ") -> {
                    when (Sharttils.config.spiritSceptreHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.spiritSceptreHider == 2)
                    }
                }

                // Giant Sword
                formatted.contains("§r§7Your Giant's Sword hit ") -> {
                    when (Sharttils.config.giantSwordHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.giantSwordHider == 2)
                    }
                }

                // Livid Dagger
                formatted.contains("§r§7Your Livid Dagger hit") -> {
                    when (Sharttils.config.lividHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.lividHider == 2)
                    }
                }

                // Ray of Hope
                formatted.startsWith("§r§7Your Ray of Hope hit") -> {
                    when (Sharttils.config.hopeHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.hopeHider == 2)
                    }
                }

                // Mining Abilities
                formatted.startsWith("§r§a§r§6") && formatted.endsWith("§r§ais now available!§r") -> {
                    when (Sharttils.config.miningAbilityHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.miningAbilityHider == 2)
                    }
                }

                // Blessings
                formatted.contains("§r§6§lDUNGEON BUFF!") -> {
                    when (Sharttils.config.blessingHider) {
                ...
                formatted.contains("§r§8Wither Key") && Utils.inDungeons -> {
                    when (Sharttils.config.witherKeyHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.witherKeyHider == 2)
                        3 -> {
                            cancelChatPacket(event, false)
                ...
                unformatted.contains("Blood Key") && Utils.inDungeons -> {
                    when (Sharttils.config.bloodKeyHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.bloodKeyHider == 2)
                        3 -> {
                            cancelChatPacket(event, false)
                            if (unformatted.contains("was picked up")) {
                ...
                formatted.contains("§r§9Superboom TNT") && Utils.inDungeons -> {
                    when (Sharttils.config.superboomHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.superboomHider == 2)
                        3 -> {
                            cancelChatPacket(event, false)
                ...
                formatted.contains("§r§6Revive Stone") && Utils.inDungeons -> {
                    when (Sharttils.config.reviveStoneHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.reviveStoneHider == 2)
                        3 -> {
                            cancelChatPacket(event, false)
                            val username = mc.thePlayer.name
                ...
                unformatted.contains(" Combo") -> {
                    when (Sharttils.config.comboHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.comboHider == 2)
                        3 -> {
                ...
                formatted.startsWith("§r§aYour Blessing enchant") -> {
                    when (Sharttils.config.blessingEnchantHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.blessingEnchantHider == 2)
                    }
                }

                // Blessing bair
                formatted.startsWith("§r§aYour bait got you double") -> {
                    when (Sharttils.config.blessedBaitHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.blessedBaitHider == 2)
                    }
                }

                // Sea creature catch
                formatted.startsWith("§r§a") && SEA_CREATURES.contains(unformatted) -> {
                    when (Sharttils.config.scCatchHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.scCatchHider == 2)
                    }
                }

                // Legendary sea creature catch
                formatted.startsWith("§r§a") && LEGENDARY_SEA_CREATURES.contains(unformatted) -> {
                    when (Sharttils.config.legendaryScCatchHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.legendaryScCatchHider == 2)
                    }
                }

                // Good catch
                unformatted.startsWith("GOOD CATCH! You found") -> {
                    when (Sharttils.config.goodTreasureHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.goodTreasureHider == 2)
                    }
                }

                // Great catch
                unformatted.startsWith("GREAT CATCH! You found") -> {
                    when (Sharttils.config.greatTreasureHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.greatTreasureHider == 2)
                    }
                }

                // Compact
                unformatted.startsWith("COMPACT! You found") -> {
                    when (Sharttils.config.compactHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.compactHider == 2)
                    }
                }

                // Blocks in the way
                unformatted.contains("There are blocks in the way") -> {
                    when (Sharttils.config.inTheWayHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.inTheWayHider == 2)
                    }
                }

                // Cooldown
                unformatted.contains(" cooldown") -> {
                    when (Sharttils.config.cooldownHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.cooldownHider == 2)
                    }
                }

                // Out of mana
                unformatted.contains("You do not have enough mana to do this!") || unformatted.startsWith("Not enough mana!") -> {
                    when (Sharttils.config.manaMessages) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.manaMessages == 2)
                    }
                }
                ...
                (Utils.inDungeons && unformatted.contains("is now available!") && !unformatted.contains("Mining Speed Boost") && !unformatted.contains(
                    ...
                    when (Sharttils.config.hideDungeonAbilities) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.hideDungeonAbilities == 2)
                    }
                }
                ...
                (Utils.inDungeons && unformatted.contains("has started the dungeon countdown. The dungeon will begin in 1 minute.") || unformatted.contains(
                    ...
                    when (Sharttils.config.hideDungeonCountdownAndReady) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.hideDungeonCountdownAndReady == 2)
                    }
                }

                // Compact Building Tools
                (Sharttils.config.compactBuildingTools && formatted.startsWith("§3Sharttils > §eThis will expire in") && (formatted.contains(
                    "blocks"
                ) || formatted.contains("build") || formatted.contains(
                    "place"
                ) || formatted.contains("zap"))
                        ) -> {
                    if (formatted.startsWith("§3Sharttils > §eThis will expire in") || Regexs.BUILDINGTOOLS.pattern.matcher(
                            formatted
                        ).matches()
                    ) {
                ...
                formatted.startsWith("§r§eYou formed a tether") -> {
                    when (Sharttils.config.tetherHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.tetherHider == 2)
                    }
                }

                // Self Orb Pickups
                formatted.startsWith("§r§c◕ §r§eYou picked up a") -> {
                    when (Sharttils.config.selfOrbHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.selfOrbHider == 2)
                    }
                }

                // Other Orb Pickups
                formatted.contains("§r§epicked up your ") -> {
                    when (Sharttils.config.otherOrbHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.otherOrbHider == 2)
                    }
                }
                ...
                formatted.startsWithAny(
                    ...
                    when (Sharttils.config.trapDamageHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.trapDamageHider == 2)
                    }
                }

                // Auto-Recombobulator
                unformatted.startsWith("Your Auto-Recomb") -> {
                    when (Sharttils.config.autoRecombHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.autoRecombHider == 2)
                        3 -> {
                ...
                formatted.contains("§r§eunlocked §r§dWither Essence §r§8x") -> {
                    when (Sharttils.config.witherEssenceHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.witherEssenceHider == 2)
                    }
                }

                // Undead Essence
                formatted.contains("§r§eunlocked §r§dUndead Essence §r§8x") -> {
                    when (Sharttils.config.undeadEssenceHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.undeadEssenceHider == 2)
                    }
                }

                // Healing (Zombie sword & Werewolf)
                formatted.startsWith("§r§a§l") && formatted.contains("healed") -> {
                    when (Sharttils.config.healingHider) {
                        1, 2 -> cancelChatPacket(event, Sharttils.config.healingHider == 2)
                    }
                }
                // Spooky Staff
                Sharttils.config.spookyMessageHider != 0 && System.currentTimeMillis() - lastSpooked <= 10000 && Regexs.SPOOKED.pattern.matcher(
                    formatted
                ).matches() -> {
                    cancelChatPacket(event, Sharttils.config.spookyMessageHider == 2)
                }
                Sharttils.config.compactPowderMessages && formatted.startsWith("§r§aYou received §r§b+") && formatted.endsWith(
                    " Powder§r"
                ) -> {
    ...
    private fun checkFilter(filter: Filter, formatted: String, unformatted: String, event: ReceiveEvent): Boolean {
        ...
        runCatching {
            ...
            UChat.chat("§cSharttils ran into an error whilst checking your Spam Hider filters. Please send your logs to discord.gg/sharttils.")
    ...
    class SpamGuiElement : GuiElement("Spam Gui", 1.0f, FloatPair(0.65f, 0.925f)) {
        ...
        override fun render() {
            ...
            while (i in spamMessages.indices) {
                ...
                val shadow: TextShadow = when (Sharttils.config.spamShadow) {
                    1 -> TextShadow.NONE
        ...
        override val toggled: Boolean
            get() = Sharttils.config.getCategoryFromSearch("Hider").items.filterIsInstance<PropertyItem>()
