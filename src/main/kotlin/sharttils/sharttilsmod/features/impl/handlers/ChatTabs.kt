...
import sharttils.sharttilsmod.gui.elements.CleanButton
import sharttils.sharttilsmod.mixins.extensions.ExtensionChatLine
import sharttils.sharttilsmod.mixins.extensions.ExtensionChatStyle
import sharttils.sharttilsmod.mixins.transformers.accessors.AccessorGuiChat
...
object ChatTabs {
    ...
    fun onChat(event: PacketEvent.ReceiveEvent) {
        if (!Utils.isOnHypixel || !Sharttils.config.chatTabs || event.packet !is S02PacketChat) return
    ...
    fun shouldAllow(component: IChatComponent): Boolean {
        if (!Utils.isOnHypixel || !Sharttils.config.chatTabs) return true
    ...
    fun onOpenGui(event: GuiOpenEvent) {
        if (!Sharttils.config.chatTabs || !Sharttils.config.preFillChatTabCommands || !Utils.isOnHypixel || event.gui !is GuiChat) return
    ...
    fun onScreenEvent(event: GuiScreenEvent) {
        if (!Sharttils.config.chatTabs || !Utils.isOnHypixel || event.gui !is GuiChat) return
        ...
        when (event) {
            ...
            is GuiScreenEvent.ActionPerformedEvent.Pre -> {
                ChatTab.buttons.entries.find {
                    ...
                    runCatching {
                        ...
                        e.printStackTrace()
                        UChat.chat("§cSharttils ran into an error while refreshing chat tabs. Please send your logs on our Discord server at discord.gg/sharttils!")
                    ...
                    if (Sharttils.config.autoSwitchChatChannel) {
                        Sharttils.sendMessageQueue.addFirst(
                            when (selectedTab) {
                                ChatTab.ALL -> "/chat a"
            ...
            is GuiScreenEvent.DrawScreenEvent.Pre -> {
                ...
                hoveredChatLine =
                    if (chat.chatOpen && Sharttils.config.copyChat) chat.getChatLine(Mouse.getX(), Mouse.getY()) else null
            }
            is GuiScreenEvent.MouseInputEvent.Pre -> {
                if (Mouse.getEventButtonState()) {
                    ...
                    } else if (Sharttils.config.copyChat) {
                        val button = Mouse.getEventButton()
                        if (button != 0) return
                        val chatLine = hoveredChatLine ?: return
