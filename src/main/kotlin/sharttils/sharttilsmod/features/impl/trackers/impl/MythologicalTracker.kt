...
package sharttils.sharttilsmod.features.impl.trackers.impl
...
class MythologicalTracker : Tracker("mythological") {
    ...
    fun onJoinWorld(event: EntityJoinWorldEvent) {
        if (lastMinosChamp != 0L && Utils.inSkyblock && mc.thePlayer != null && Sharttils.config.trackMythEvent && event.entity is EntityOtherPlayerMP && event.entity.getXZDistSq(
                mc.thePlayer
            ...
            if (event.entity.name == "Minos Champion") {
                ...
                UChat.chat("§bSharttils: §eYou dug up a §2Minos Champion§e!")
                ...
                UChat.chat("§bSharttils: §eYou dug up a §2Minos Inquisitor§e!")
                markDirty<MythologicalTracker>()
            }
        }
        if (lastMinosChamp != 0L && System.currentTimeMillis() - lastMinosChamp > 2500) {
            ...
            lastMinosChamp = 0L
            BurrowMob.CHAMP.dugTimes++
            UChat.chat("§bSharttils: §eNo idea what you dug, counting as §2Minos Champion§e!")
            markDirty<MythologicalTracker>()
        }
    }
    ...
    fun onReceivePacket(event: PacketEvent.ReceiveEvent) {
        if (!Utils.inSkyblock || (!Sharttils.config.trackMythEvent && !Sharttils.config.broadcastMythCreatureDrop)) return
        when (event.packet) {
            is S02PacketChat -> {
                if (event.packet.type == 2.toByte() || !Sharttils.config.trackMythEvent) return
            ...
            is S2FPacketSetSlot -> {
                ...
                if (Sharttils.config.broadcastMythCreatureDrop) {
                    val text = "§6§lRARE DROP! ${drop.rarity.baseColor}${drop.itemName} §b(Sharttils User Luck!)"
                    if (Sharttils.config.autoCopyRNGDrops) GuiScreen.setClipboardString(text)
                    UChat.chat(text)
                ...
                if (Sharttils.config.trackMythEvent) {
                    drop.droppedTimes++
                    markDirty<MythologicalTracker>()
                }
            }
    ...
    class MythologicalTrackerElement : GuiElement("Mythological Tracker", FloatPair(150, 120)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.trackMythEvent
