...
object DupeTracker : Tracker("duped_items") {
    ...
    fun onPacket(event: MainReceivePacketEvent<*, *>) {
        when (val packet = event.packet) {
            ...
            is S2FPacketSetSlot -> {
                if (!inAuctionBrowser || !Sharttils.config.dupeTracker) return
    ...
    fun onSlotDraw(event: GuiContainerEvent.DrawSlotEvent.Post) {
        if (!Utils.inSkyblock || !Sharttils.config.dupeTracker) return
        ...
        if (dupedUUIDs.contains(uuid) || dirtyUUIDs.contains(uuid)) {
            ...
            event.slot highlight Sharttils.config.dupeTrackerOverlayColor
    ...
    fun onTooltip(event: ItemTooltipEvent) {
        ...
        if (Sharttils.config.dupeTracker) {
        ...
        when (origin) {
            ...
            else -> if (Sharttils.config.showOrigin) event.toolTip.add("§7§lOrigin: $origin")
        }
    }
    ...
    init {
        fixedRateTimer(name = "Sharttils-FetchDupeData", period = 7 * 60 * 1000L) {
            if (Utils.skyblock && Sharttils.config.dupeTracker) {
                APIUtil.getArrayResponse("https://${Sharttils.domain}/api/auctions/dupeditems").apply {
                    Utils.checkThreadAndQueue {
                        dupedUUIDs.addAll(map { it.asString })
                    }
                }
                if (Sharttils.config.markDirtyItems) {
                    APIUtil.getArrayResponse("https://${Sharttils.domain}/api/auctions/dirtyitems").apply {
                        Utils.checkThreadAndQueue {
