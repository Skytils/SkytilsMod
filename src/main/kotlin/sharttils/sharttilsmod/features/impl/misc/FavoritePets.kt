...
import sharttils.sharttilsmod.utils.ItemRarity
...
class FavoritePets : PersistentSave(File(Sharttils.modDir, "favoritepets.json")) {
    ...
    fun onGuiScreenEvent(event: GuiScreenEvent) {
        if (!Utils.inSkyblock || !Sharttils.config.highlightFavoritePets) return
    ...
    fun onSlotDraw(event: GuiContainerEvent.DrawSlotEvent.Pre) {
        if (!Utils.inSkyblock || !Sharttils.config.highlightFavoritePets || event.container !is ContainerChest) return
        val chest = event.container
        if (!chest.lowerChestInventory.name.endsWith(") Pets") && chest.lowerChestInventory.name != "Pets") return
        ...
        if (favorited.contains(petId)) {
            ...
            event.slot highlight Sharttils.config.favoritePetColor
            GlStateManager.translate(0f, 0f, -2f)
        }
    }
