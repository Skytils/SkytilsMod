...
import sharttils.sharttilsmod.events.impl.*
...
import sharttils.sharttilsmod.utils.NumberUtil.romanToDecimal
import sharttils.sharttilsmod.utils.NumberUtil.roundToPrecision
...
class MiscFeatures {
    ...
    fun onSendChatMessage(event: SendChatMessageEvent) {
        ...
        if (Sharttils.config.guildLeaveConfirmation && event.message.startsWith("/g leave") && System.currentTimeMillis() - lastGLeaveCommand >= 10_000) {
            event.isCanceled = true
            lastGLeaveCommand = System.currentTimeMillis()
            UChat.chat("§cSharttils stopped you from using leaving your guild! §6Run the command again if you wish to leave!")
        }
        if (Sharttils.config.coopAddConfirmation && event.message.startsWith("/coopadd ") && System.currentTimeMillis() - lastCoopAddCommand >= 10_000) {
            event.isCanceled = true
            lastCoopAddCommand = System.currentTimeMillis()
            UChat.chat("§c§lBe careful! Sharttils stopped you from giving a player full control of your island! §6Run the command again if you are sure!")
        }
    }
    ...
    fun onBossBarSet(event: BossBarEvent.Set) {
        ...
        if (Utils.inSkyblock) {
            if (Sharttils.config.bossBarFix && equalsOneOf(
    ...
    fun onChat(event: ClientChatReceivedEvent) {
        ...
        if (Sharttils.config.blockZapperFatigueTimer) {
            ...
            if (unformatted == "Your zapper is temporarily fatigued!") {
                ...
                if (duration.isPositive()) {
                    UChat.chat("§3Sharttils > §eThis will expire in${
                        duration
        ...
        if (!Utils.inDungeons) {
            if (Sharttils.config.copyDeathToClipboard) {
                if (formatted.startsWith("§r§c ☠ ")) {
                    event.message.chatStyle
                        ...
                        ClickEvent(ClickEvent.Action.RUN_COMMAND, "/sharttilscopy $unformatted")

                }
            }
        }
        if (Utils.inSkyblock) {
            if (Sharttils.config.autoCopyRNGDrops) {
                if (formatted.startsWith("§r§d§lCRAZY RARE DROP! ") || formatted.startsWith("§r§c§lINSANE DROP! ") || formatted.startsWith(
                    ...
                    UChat.chat("§9§lSharttils §8» §aCopied RNG drop to clipboard.")
                    event.message.chatStyle
                        .setChatHoverEvent(
                            HoverEvent(
                                HoverEvent.Action.SHOW_TEXT,
            ...
            if (Sharttils.config.autoCopyVeryRareDrops) {
                if (formatted.startsWith("§r§9§lVERY RARE DROP! ") || formatted.startsWith("§r§5§lVERY RARE DROP! ")) {
                    GuiScreen.setClipboardString(unformatted)
                    UChat.chat("§9§lSharttils §8» §aCopied very rare drop to clipboard.")
                    event.message.chatStyle
                        .setChatHoverEvent(
                            HoverEvent(
                                HoverEvent.Action.SHOW_TEXT,
    ...
    fun onCheckRender(event: CheckRenderEntityEvent<*>) {
        if (!Utils.inSkyblock) return
        if (Sharttils.config.hideDyingMobs && event.entity is EntityLivingBase && (event.entity.health <= 0 || event.entity.isDead)) {
            event.isCanceled = true
        } else if (event.entity is EntityFallingBlock) {
            val entity = event.entity
            if (Sharttils.config.hideMidasStaffGoldBlocks && entity.block.block === Blocks.gold_block) {
                event.isCanceled = true
            }
        } else if (event.entity is EntityItem) {
            val entity = event.entity
            val item = entity.entityItem
            if (Sharttils.config.hideJerryRune) {
            ...
            if (Sharttils.config.hideCheapCoins && cheapCoins.contains(ItemUtil.getSkullTexture(item))) {
                event.isCanceled = true
            }
        } else if (event.entity is EntityLightningBolt) {
            if (Sharttils.config.hideLightning) {
                event.isCanceled = true
            }
        } else if (Sharttils.deobfEnvironment && DevTools.getToggle("invis")) {
    ...
    fun onDrawSlot(event: GuiContainerEvent.DrawSlotEvent.Pre) {
        ...
        if (Sharttils.config.highlightDisabledPotionEffects && event.slot.hasStack && SBInfo.lastOpenContainerName?.startsWith(
    ...
    fun onJoin(event: EntityJoinWorldEvent) {
        ...
        if (event.entity is EntityArmorStand) {
            TickTask(5) {
                ...
                if (Sharttils.config.trickOrTreatChestAlert && mc.thePlayer != null && headSlot != null && headSlot.item === Items.skull && headSlot.hasTagCompound() && entity.getDistanceSqToEntity(
                        mc.thePlayer
                    ) < 10 * 10
                ) {
    ...
    fun onRenderOverlayPre(event: RenderGameOverlayEvent.Pre) {
        if (!Utils.inSkyblock) return
        if (event.type == RenderGameOverlayEvent.ElementType.AIR && Sharttils.config.hideAirDisplay && !Utils.inDungeons) {
            event.isCanceled = true
        }
    }
    ...
    fun onRenderBlockOverlay(event: RenderBlockOverlayEvent) {
        if (Utils.inSkyblock && Sharttils.config.noFire && event.overlayType == RenderBlockOverlayEvent.OverlayType.FIRE) {
            event.isCanceled = true
        }
    }
    ...
    fun onReceivePacket(event: ReceiveEvent) {
        ...
        if (event.packet is S29PacketSoundEffect) {
            ...
            if (Sharttils.config.disableCooldownSounds && packet.soundName == "mob.endermen.portal" && packet.pitch == 0f && packet.volume == 8f) {
                event.isCanceled = true
                return
            }
            if (Sharttils.config.disableJerrygunSounds) {
            ...
            if (Sharttils.config.disableTruthFlowerSounds && packet.soundName == "random.eat" && packet.pitch == 0.6984127f && packet.volume == 1.0f) {
                event.isCanceled = true
                return
            }
        }
    ...
    fun onSlotClick(event: SlotClickEvent) {
        ...
        if (event.container is ContainerChest) {
            ...
            if (inventoryName == "Ophelia") {
                if (Sharttils.config.dungeonPotLock > 0) {
                    ...
                    if (extraAttributes.getInteger("potion_level") != Sharttils.config.dungeonPotLock) {
                        event.isCanceled = true
                        return
                    }
                }
    ...
    fun onTooltip(event: ItemTooltipEvent) {
        if (!Utils.inSkyblock) return
        if (!Sharttils.config.hideTooltipsOnStorage) return
    ...
    fun onSlotClickLow(event: SlotClickEvent) {
        if (!Utils.inSkyblock || !Sharttils.config.middleClickGUIItems) return
    ...
    fun onTick(event: TickEvent.ClientTickEvent) {
        ...
        if (Sharttils.config.legionPlayerDisplay) {
        ...
        if (Sharttils.config.summoningEyeDisplay && SBInfo.mode == SkyblockIsland.TheEnd.mode) {
    ...
    fun onRenderItemOverlayPost(event: GuiRenderItemEvent.RenderOverlayEvent.Post) {
        ...
        if (!Utils.inSkyblock || mc.thePlayer == null || item.stackSize != 1 || item.tagCompound?.hasKey("SharttilsNoItemOverlay") == true) return
        var stackTip = ""

        val c = mc.thePlayer.openContainer
        if (c is ContainerChest) {
            val name = c.lowerChestInventory.name
            if (Sharttils.config.showBestiaryLevel && name.startsWithAny(
                    "Bestiary ➜ ",
                    "Search Results"
            ...
            if (Sharttils.config.showDungeonFloorAsStackSize && name == "Catacombs Gate" && item.item === Items.skull) {
                stackTip =
    ...
    class GolemSpawnTimerElement : GuiElement("Endstone Protector Spawn Timer", FloatPair(150, 20)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.golemSpawnTimer
    ...
    class LegionPlayerDisplay : GuiElement("Legion Player Display", FloatPair(50, 50)) {
        override fun render() {
            if (hasLegion && toggled && Utils.inSkyblock && mc.thePlayer != null && mc.theWorld != null) {
                ...
                ScreenRenderer.fontRenderer.drawString(
                    if (Sharttils.config.legionCap && legionPlayers > 20) "20" else legionPlayers.toString(),
                    20f,
                    5f,
                    CommonColors.ORANGE,
                    TextAlignment.LEFT_RIGHT,
        ...
        override val toggled: Boolean
            get() = Sharttils.config.legionPlayerDisplay
    ...
    class PlacedSummoningEyeDisplay : GuiElement("Placed Summoning Eye Display", FloatPair(50, 60)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.summoningEyeDisplay
        ...
        companion object {
            ...
            private val ICON = ResourceLocation("sharttils", "icons/SUMMONING_EYE.png")
        }
    ...
    class WorldAgeDisplay : GuiElement("World Age Display", FloatPair(50, 60)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.showWorldAge
    ...
    object ItemNameHighlightDummy : GuiElement("Item Name Highlight", FloatPair(50, 60)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.moveableItemNameHighlight
    ...
    object ActionBarDummy : GuiElement("Action Bar", FloatPair(50, 70)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.moveableActionBar
