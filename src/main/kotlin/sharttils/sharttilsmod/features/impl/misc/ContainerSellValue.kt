...
import sharttils.sharttilsmod.utils.Utils.inDungeons
...
object ContainerSellValue {
    ...
    class SellValueDisplay : GuiElement("Container Sell Value", FloatPair(0.258f, 0.283f)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.containerSellValue
        override val height: Int = fr.FONT_HEIGHT * (Sharttils.config.containerSellValueMaxItems + 2)
    ...
    private fun getItemValue(itemStack: ItemStack): Double {
        ...
        if(itemStack.stackSize > 1 || !Sharttils.config.includeModifiersInSellValue) return basePrice * itemStack.stackSize
    ...
    private fun shouldRenderGuiComponent() : Boolean {
        ...
        return Sharttils.config.containerSellValue && gui is GuiChest && isChestNameValid(chestName)
    }
     ...
     *
     * Note: It is important that Minecraft Forge's `BackgroundDrawnEvent` is used instead of Sharttils's
     * GuiContainerEvent.BackgroundDrawnEvent because Sharttils's event is called before the overlay rectangle is drawn,
    ...
    fun renderGuiComponent() {
        ...
        if (lines > Sharttils.config.containerSellValueMaxItems) {
            drawLine(stack, textLines.size, "§7and ${lines - Sharttils.config.containerSellValueMaxItems} more...")
    ...
    fun onTick(event: TickEvent.ClientTickEvent) {
        if(!Sharttils.config.containerSellValue) return
        ...
        textLines.addAll(
            ...
            .toList()
            .also { lines = it.size }
            .take(Sharttils.config.containerSellValueMaxItems)
        )
    }
