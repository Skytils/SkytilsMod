...
import sharttils.sharttilsmod.events.impl.RenderHUDEvent
import sharttils.sharttilsmod.features.impl.handlers.MayorInfo
import sharttils.sharttilsmod.mixins.transformers.accessors.AccessorMinecraft
import sharttils.sharttilsmod.utils.*
import sharttils.sharttilsmod.utils.NumberUtil.roundToPrecision
import sharttils.sharttilsmod.utils.NumberUtil.toRoman
import sharttils.sharttilsmod.utils.RenderUtil.drawFilledBoundingBox
import sharttils.sharttilsmod.utils.RenderUtil.drawOutlinedBoundingBox
import sharttils.sharttilsmod.utils.ScoreboardUtil.sidebarLines
...
class SlayerFeatures {
    ...
    fun onChat(event: ClientChatReceivedEvent) {
        ...
        if (Sharttils.config.openMaddoxMenu) {
            if (formatted.contains("§2§l[OPEN MENU]")) {
                ...
                maddoxCommand =
                    listOfSiblings.find { it.unformattedText.contains("[OPEN MENU]") }?.chatStyle?.chatClickEvent?.value
                        ?: ""
                mc.thePlayer.addChatMessage(ChatComponentText(EnumChatFormatting.LIGHT_PURPLE.toString() + "Sharttils: Open chat then click anywhere on screen to open Maddox Menu."))
            }
        }
    }
    ...
    fun onMouseInputPost(event: GuiScreenEvent.MouseInputEvent.Post) {
        ...
        if (Mouse.getEventButton() == 0 && event.gui is GuiChat) {
            if (Sharttils.config.openMaddoxMenu && maddoxCommand.isNotBlank()) {
                Sharttils.sendMessageQueue.add(maddoxCommand)
                maddoxCommand = ""
            }
        }
    }
    ...
    fun onTick(event: ClientTickEvent) {
        ...
        if (Sharttils.config.experimentalYangGlyphDetection && lastYangGlyphSwitchTicks >= 0 && yangGlyphEntity == null && yangGlyph == null && slayerEntity != null) {
        ...
        if (ticks % 4 == 0) {
            if (Sharttils.config.rev5TNTPing) {
                if (hasSlayerText) {
            ...
            if (Sharttils.config.showRNGMeter) {
                    ...
                    if (line == "Slayer Quest") {
                        ...
                        if (boss.startsWith("Revenant Horror")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    100f,
                                    Sharttils.config.revRNG,
                                    ChatComponentText("§2§lRevenant Horror RNG§r - §d${Sharttils.config.revRNG}%")
                                ), true
                            )
                            break
                        }
                        if (boss.startsWith("Tarantula Broodfather")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    100f,
                                    Sharttils.config.taraRNG,
                                    ChatComponentText("§8§lTarantula Broodfather RNG§r - §d${Sharttils.config.taraRNG}%")
                                ), true
                            )
                            break
                        }
                        if (boss.startsWith("Sven Packmaster")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    100f,
                                    Sharttils.config.svenRNG,
                                    ChatComponentText("§7§lSven Packmaster RNG§r - §d${Sharttils.config.svenRNG}%")
                                ), true
                            )
                            break
                        }
                        if (boss.startsWith("Voidgloom Seraph")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    100f,
                                    Sharttils.config.voidRNG,
                                    ChatComponentText("§5§lVoidgloom Seraph RNG§r - §d${Sharttils.config.voidRNG}%")
                                ), true
                            )
                            break
                        }
    ...
    fun onRenderLivingPre(event: RenderLivingEvent.Pre<EntityLivingBase>) {
        ...
        if (event.entity is EntityArmorStand) {
            ...
            if (Sharttils.config.slayerBossHitbox && name.endsWith("§c❤") && !name.endsWith("§e0§c❤") && !mc.renderManager.isDebugBoundingBox) {
                val x =
    ...
    fun onReceivePacket(event: ReceiveEvent) {
            ...
            if (packet.entityId == slayerEntity?.entityId && slayerEntity is EntityEnderman) {
                (slayerEntity as EntityEnderman).apply {
                    if (heldBlockState?.block == Blocks.beacon && ((packet.func_149376_c()
                        ...
                        if (Sharttils.config.yangGlyphPing && !Sharttils.config.yangGlyphPingOnLand) createTitle(
                            "§cYang Glyph!",
                            30
                        )
        ...
        if (packet is S29PacketSoundEffect) {
            if (Sharttils.config.slayerMinibossSpawnAlert && slayerEntity == null && packet.soundName == "random.explode" && packet.volume == 0.6f && packet.pitch == 9 / 7f && GuiManager.title != "§cMINIBOSS" && sidebarLines.any {
            ...
            if (unformatted.trim().startsWith("RNGesus Meter")) {
                ...
                for ((index, line) in sidebarLines.withIndex()) {
                    if (line == "Slayer Quest") {
                        val boss = sidebarLines.elementAtOrNull(index - 1) ?: continue
                        if (boss.startsWith("Revenant Horror")) {
                            Sharttils.config.revRNG = rngMeter
                            break
                        }
                        if (boss.startsWith("Tarantula Broodfather")) {
                            Sharttils.config.taraRNG = rngMeter
                            break
                        }
                        if (boss.startsWith("Sven Packmaster")) {
                            Sharttils.config.svenRNG = rngMeter
                            break
                        }
                        if (boss.startsWith("Voidgloom Seraph")) {
                            Sharttils.config.voidRNG = rngMeter
                            break
                        }
                    }
                }
    ...
    fun onBlockChange(event: BlockChangeEvent) {
        ...
        if (event.update.block is BlockBeacon) {
            ...
            if (Sharttils.config.experimentalYangGlyphDetection && yangGlyph == null && slayerEntity != null) {
    ...
    fun onWorldRender(event: RenderWorldLastEvent) {
        if (!Utils.inSkyblock) return
        if (Sharttils.config.highlightYangGlyph && yangGlyph != null) {
            ...
            drawFilledBoundingBox(
                ...
                Sharttils.config.yangGlyphColor,
                1f
            )
            GlStateManager.enableDepth()
            GlStateManager.enableCull()
        }
        if (Sharttils.config.highlightNukekebiHeads && nukekebiHeads.isNotEmpty()) {
            for (head in nukekebiHeads.also { list -> list.removeAll { it.isDead } }) {
                ...
                drawFilledBoundingBox(
                    ...
                    Sharttils.config.nukekebiHeadColor,
                    1f
                )
                GlStateManager.enableDepth()
                GlStateManager.enableCull()
    ...
    fun onClick(event: InputEvent.MouseInputEvent) {
        if (!Utils.inSkyblock || mc.pointedEntity == null || Sharttils.config.slayerCarryMode == 0 || Mouse.getEventButton() != 2 || !Mouse.getEventButtonState() || mc.currentScreen != null || mc.thePlayer == null) return
        processSlayerEntity(mc.pointedEntity, false)
    }

    @SubscribeEvent
    fun onAttack(event: AttackEntityEvent) {
        if (!hasSlayerText || !Utils.inSkyblock || event.entity != mc.thePlayer || event.target !is EntityEnderman || !Sharttils.config.useSlayerHitMethod) return
    ...
    fun onCheckRender(event: CheckRenderEntityEvent<*>) {
        if (!Sharttils.config.hideOthersBrokenHeartRadiation || !event.entity.isInvisible || event.entity !is EntityGuardian) return
    ...
    fun onRenderHud(event: RenderHUDEvent) {
        if (!Utils.inSkyblock) return
        if (Sharttils.config.pointYangGlyph) {
            ...
            Sharttils.config.yangGlyphColor.withAlpha(255).bindColor()
            GL11.glLineWidth(5f)
    ...
    class SlayerDisplayElement : GuiElement("Slayer Display", FloatPair(150, 20)) {
            ...
            if (Utils.inSkyblock) {
                ...
                if (slayerEntity != null) {
                    if (slayerEntity!!.isDead) {
                        ...
                        if (Sharttils.config.slayerTimeToKill) {
                            mc.thePlayer.addChatComponentMessage(ChatComponentText("§9§lSharttils §8» §bSlayer took §f${slayerEntity!!.ticksExisted / 20f}§bs to kill"))
                        }
                        slayerEntity = null
                        nukekebiHeads.clear()
                        lastYangGlyphSwitch = -1L
        ...
        override val toggled: Boolean
            get() = Sharttils.config.showSlayerDisplay
    ...
    class SeraphDisplayElement : GuiElement("Seraph Display", FloatPair(20, 20)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.showSeraphDisplay
    ...
    class SlayerArmorDisplayElement : GuiElement("Slayer Armor Display", FloatPair(150, 20)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.showSlayerArmorKills
    ...
    companion object {
        ...
        private fun getTier(name: String): String {
            return sidebarLines.find { it.startsWith(name) }?.substringAfter(name)?.drop(1)
                ?: (if (Sharttils.config.slayerCarryMode > 0) Sharttils.config.slayerCarryMode.toRoman() else "")
        }
