...
import sharttils.sharttilsmod.mixins.transformers.accessors.AccessorGuiContainer
import sharttils.sharttilsmod.utils.*
import sharttils.sharttilsmod.utils.ItemUtil.getDisplayName
...
class ItemFeatures {
    ...
    fun onTick(event: TickEvent.ClientTickEvent) {
        if (ticks % 4 == 0) {
            if (mc.thePlayer != null && Utils.inSkyblock) {
                ...
                if (Sharttils.config.stackingEnchantProgressDisplay) {
                    apply {
                        also {
    ...
    fun onDrawSlot(event: GuiContainerEvent.DrawSlotEvent.Pre) {
        if (Utils.inSkyblock && Sharttils.config.showItemRarity && event.slot.hasStack) {
        ...
        if (event.container is ContainerChest) {
            ...
            if (chestName.startsWithAny("Salvage", "Ender Chest") || Utils.equalsOneOf(
                ...
                if (Sharttils.config.highlightSalvageableItems) {
            ...
            if (chestName == "Ophelia" || chestName == "Trades") {
                if (Sharttils.config.highlightDungeonSellableItems) {
                    if (event.slot.hasStack) {
                        val stack = event.slot.stack
    ...
    fun onSlotClick(event: SlotClickEvent) {
        ...
        if (event.container is ContainerChest) {
            ...
            if (event.slot != null && event.slot.hasStack) {
                ...
                if (Sharttils.config.stopClickingNonSalvageable) {
    ...
    fun onTooltip(event: ItemTooltipEvent) {
        ...
        if (itemId != null) {
            if (Sharttils.config.showLowestBINPrice || Sharttils.config.showCoinsPerBit) {
                ...
                if (auctionIdentifier != null) {
                    ...
                    if (valuePer != null) {
                        if (Sharttils.config.showLowestBINPrice) {
                            val total =
                        ...
                        if (Sharttils.config.showCoinsPerBit) {
            ...
            if (Sharttils.config.showNPCSellPrice) {
        ...
        if (Sharttils.config.showRadioactiveBonus && itemId == "TARANTULA_HELMET") {
            val bonus = try {
        ...
        if (Sharttils.config.showGemstones && extraAttr?.hasKey("gems") == true) {
    ...
    fun onReceivePacket(event: MainReceivePacketEvent<*, *>) {
        ...
        event.packet.apply {
            if (this is S2APacketParticles) {
                if (type == EnumParticleTypes.EXPLOSION_LARGE && Sharttils.config.hideImplosionParticles) {
                ...
                if (Utils.equalsOneOf(itemId, "SOULFLOW_PILE", "SOULFLOW_BATTERY", "SOULFLOW_SUPERCELL")) {
                    getItemLore(item).find {
                        ...
                        s.drop(2).filter { it.isDigit() }.toIntOrNull()?.let {
                            if (Sharttils.config.lowSoulflowPing > 0) {
                                if (it <= Sharttils.config.lowSoulflowPing && !lowSoulFlowPinged) {
                                    GuiManager.createTitle("§cLow Soulflow", 20)
                                    lowSoulFlowPinged = true
                                } else if (it > Sharttils.config.lowSoulflowPing) {
    ...
    fun onEntitySpawn(event: EntityJoinWorldEvent) {
        if (!Utils.inSkyblock) return
        if (event.entity !is EntityFishHook || !Sharttils.config.hideFishingHooks) return
    ...
    fun onInteract(event: PlayerInteractEvent) {
        ...
        if (Sharttils.config.preventPlacingWeapons && event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK && (Utils.equalsOneOf(
                itemId,
                "FLOWER_OF_TRUTH",
                "BAT_WAND",
                "WEIRD_TUBA"
    ...
    fun onRenderItemOverlayPost(event: GuiRenderItemEvent.RenderOverlayEvent.Post) {
        ...
        if (extraAttributes != null) {
            if (Sharttils.config.showPotionTier && extraAttributes.hasKey("potion_level")) {
                stackTip = extraAttributes.getInteger("potion_level").toString()
            } else if ((Sharttils.config.showEnchantedBookTier || Sharttils.config.showEnchantedBookAbbreviation) && item.item === Items.enchanted_book && extraAttributes.hasKey(
                    "enchantments"
                )
            ) {
                ...
                if (enchantments.keySet.size == 1) {
                    ...
                    if (Sharttils.config.showEnchantedBookAbbreviation) {
                    ...
                    if (Sharttils.config.showEnchantedBookTier) stackTip =
                        enchantments.getInteger(name.toString()).toString()
                }
            } else if (Sharttils.config.showDungeonItemLevel && extraAttributes.hasKey("dungeon_item_level")) {
        ...
        val lore = getItemLore(item)
        if (lore.isNotEmpty()) {
            if (Sharttils.config.showPetCandies && item.item === Items.skull) {
                for (line in lore) {
    ...
    fun onDrawContainerForeground(event: GuiContainerEvent.ForegroundDrawnEvent) {
        if (!Sharttils.config.bookHelper || !Utils.inSkyblock) return
    ...
    fun onRenderWorld(event: RenderWorldLastEvent) {
        if (!Utils.inSkyblock) return
        if (Sharttils.config.showEtherwarpTeleportPos && mc.thePlayer?.isSneaking == true) {
            ...
            if (isValidEtherwarpPos(obj)) {
                ...
                RenderUtil.drawFilledBoundingBox(
                    ...
                    Sharttils.config.showEtherwarpTeleportPosColor
                )
                GlStateManager.disableBlend()
    ...
    class SelectedArrowDisplay : GuiElement("Arrow Swapper Display", FloatPair(0.65f, 0.85f)) {
        ...
        override val width: Int
            get() = ScreenRenderer.fontRenderer.getStringWidth("§aSelected: §rSharttils Arrow")
        override val toggled: Boolean
            get() = Sharttils.config.showSelectedArrowDisplay
    ...
    class StackingEnchantDisplay : GuiElement("Stacking Enchant Display", FloatPair(0.65f, 0.85f)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.stackingEnchantProgressDisplay
    ...
    class SoulflowGuiElement : GuiElement("Soulflow Display", FloatPair(0.65f, 0.85f)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.showSoulflowDisplay
    ...
    class WitherShieldDisplay : GuiElement("Wither Shield Display", FloatPair(0.65f, 0.85f)) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.witherShieldCooldown
