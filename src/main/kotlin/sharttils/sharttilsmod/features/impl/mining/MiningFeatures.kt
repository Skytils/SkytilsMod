 ...
 */
package sharttils.sharttilsmod.features.impl.mining
...
import sharttils.sharttilsmod.events.impl.BossBarEvent
...
class MiningFeatures {
    ...
    fun onBossBar(event: BossBarEvent.Set) {
        ...
        if (Sharttils.config.raffleWarning) {
    ...
    fun onReceivePacket(event: PacketEvent.ReceiveEvent) {
        if (!Utils.inSkyblock) return
        if (Sharttils.config.crystalHollowDeathWaypoint && event.packet is S08PacketPlayerPosLook && mc.thePlayer != null) {
    ...
    fun onChat(event: ClientChatReceivedEvent) {
        ...
        if (Sharttils.config.puzzlerSolver && unformatted.startsWith("[NPC] Puzzler:")) {
        ...
        if (Sharttils.config.fetchurSolver && unformatted.startsWith("[NPC] Fetchur:")) {
            ...
            TickTask(50) {
                if (solution != null) {
                    ...
                    if (unformatted.contains("its") || unformatted.contains("theyre")) {
                        ...
                        UChat.chat("§cSharttils couldn't determine the Fetchur item. There were ${fetchurItems.size} solutions loaded.")
                    }
                }
            }
        }
        if (Sharttils.config.hollowChatCoords && SBInfo.mode == SkyblockIsland.CrystalHollows.mode) {
        ...
        if ((Sharttils.config.crystalHollowWaypoints || Sharttils.config.crystalHollowMapPlaces) && Sharttils.config.kingYolkarWaypoint && SBInfo.mode == SkyblockIsland.CrystalHollows.mode
        ...
        if (formatted.startsWith("§r§cYou died")) {
            ...
            if (Sharttils.config.crystalHollowDeathWaypoint && SBInfo.mode == SkyblockIsland.CrystalHollows.mode && lastTPLoc != null) {
                mc.ingameGUI.chatGUI.printChatMessage(ChatComponentText("§3Sharttils > §eClick to set a death waypoint at ${lastTPLoc!!.x} ${lastTPLoc!!.y} ${lastTPLoc!!.z}").apply {
                    chatStyle.chatClickEvent = ClickEvent(
                        ClickEvent.Action.RUN_COMMAND,
                        "/sharttilshollowwaypoint set last_death ${lastTPLoc!!.x} ${lastTPLoc!!.y} ${lastTPLoc!!.z}"
                    )
                })
            }
        }
    ...
    private fun waypointChatMessage(x: String, y: String, z: String) {
        val message = UMessage(
            "§3Sharttils > §eFound coordinates in a chat message, click a button to set a waypoint.\n"
        )
        for (loc in CrystalHollowsMap.Locations.values()) {
            if (loc.loc.exists()) continue
            message.append(
                UTextComponent("${loc.displayName} ")
                    .setClick(MCClickEventAction.SUGGEST_COMMAND, "/sharttilshollowwaypoint set ${loc.id} $x $y $z")
        ...
        message.append(
            UTextComponent("§e[Custom]").setClick(
                ...
                "/sharttilshollowwaypoint set name_here $x $y $z"
    ...
    fun onDrawSlot(event: GuiContainerEvent.DrawSlotEvent.Pre) {
        ...
        if (event.slot.hasStack) {
            ...
            if (Sharttils.config.highlightDisabledHOTMPerks && SBInfo.lastOpenContainerName == "Heart of the Mountain") {
                if (ItemUtil.getItemLore(item).any { it == "§cDISABLED" }) {
                    event.slot highlight Color(255, 0, 0)
                }
            }
            if (Sharttils.config.highlightCompletedCommissions && SBInfo.lastOpenContainerName.equals("Commissions")) {
    ...
    fun onPlayerInteract(event: PlayerInteractEvent) {
        ...
        if (event.action != PlayerInteractEvent.Action.LEFT_CLICK_BLOCK) {
            if (Sharttils.config.raffleWaypoint && inRaffle && event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
    ...
    fun onRenderWorld(event: RenderWorldLastEvent) {
        ...
        if (Sharttils.config.puzzlerSolver && puzzlerSolution != null) {
        ...
        if (Sharttils.config.raffleWaypoint && inRaffle && raffleBox != null) {
        ...
        if (Sharttils.config.crystalHollowWaypoints && SBInfo.mode == SkyblockIsland.CrystalHollows.mode) {
            GlStateManager.disableDepth()
            for (loc in CrystalHollowsMap.Locations.values()) {
    ...
    fun onRenderLivingPre(event: RenderLivingEvent.Pre<EntityLivingBase?>) {
        if (!Utils.inSkyblock) return
        if (Sharttils.config.crystalHollowWaypoints && event.entity is EntityOtherPlayerMP && event.entity.name == "Team Treasurite" && event.entity.baseMaxHealth == if (MayorInfo.mayorPerks.contains(
                    "DOUBLE MOBS HP!!!"
                )
    ...
    fun onScoreboardChange(event: PacketEvent.ReceiveEvent) {
        ...
        if (
            ...
            Sharttils.config.skymallReminder && SBInfo.mode == SkyblockIsland.DwarvenMines.mode
        ) {
    ...
    fun onTick(event: ClientTickEvent) {
        ...
        if ((Sharttils.config.crystalHollowWaypoints || Sharttils.config.crystalHollowMapPlaces) && SBInfo.mode == SkyblockIsland.CrystalHollows.mode
    ...
    fun onGetBlockModel(event: RenderBlockInWorldEvent) {
        ...
        if (Sharttils.config.recolorCarpets && state.block === Blocks.carpet && Utils.equalsOneOf(
                state.getValue(
                    BlockCarpet.COLOR
    ...
    class CrystalHollowsMap : GuiElement(name = "Crystal Hollows Map", fp = FloatPair(0, 0)) {
        val mapLocation = ResourceLocation("sharttils", "crystalhollowsmap.png")
        ...
        override fun render() {
            ...
            UMatrixStack.Compat.runLegacyMethod(stack) {
                ...
                stack.runWithGlobalState {
                    ...
                    if (Sharttils.config.crystalHollowMapPlaces) {
        ...
        override val toggled: Boolean
            get() = Sharttils.config.crystalHollowMap
