...
object StupidTreasureChestOpeningThing {
    ...
    fun onBlockChange(event: BlockChangeEvent) {
        if (!Sharttils.config.chTreasureHelper || mc.thePlayer == null || SBInfo.mode != SkyblockIsland.CrystalHollows.mode) return
    ...
    fun onSendPacket(event: PacketEvent.SendEvent) {
        if (sendHelpPlease.isEmpty() || !Sharttils.config.chTreasureHelper || SBInfo.mode != SkyblockIsland.CrystalHollows.mode) return
    ...
    fun onReceivePacket(event: PacketEvent.ReceiveEvent) {
        if (!Sharttils.config.chTreasureHelper || SBInfo.mode != SkyblockIsland.CrystalHollows.mode) return
    ...
    fun onRender(event: RenderWorldLastEvent) {
        if (!Sharttils.config.chTreasureHelper || sendHelpPlease.isEmpty() || SBInfo.mode != SkyblockIsland.CrystalHollows.mode) return
