...
fun injectScoreboardScoreRemover() = modify("net/minecraft/client/gui/GuiIngame") {
    classNode.methods.find {
        ...
        instructions.insert(InsnListBuilder(this).apply {
            ...
            checkVar = istore()
            val label = makeLabel()
            getStatic("sharttils/sharttilsmod/utils/Utils", "isOnHypixel", "Z")
            jump(JumpCondition.EQUAL, label)
            getKObjectInstance("sharttils/sharttilsmod/core/Config")
            invokeVirtual("sharttils/sharttilsmod/core/Config", "getHideScoreboardScore", "()Z")
            jump(JumpCondition.EQUAL, label)
