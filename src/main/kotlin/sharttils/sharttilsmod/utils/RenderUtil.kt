...
import sharttils.sharttilsmod.mixins.hooks.renderer.skipGlint
...
object RenderUtil {
    private val RARITY = ResourceLocation("sharttils", "gui/rarity.png")
    private val RARITY2 = ResourceLocation("sharttils", "gui/rarity2.png")
    private val RARITY3 = ResourceLocation("sharttils", "gui/rarity3.png")
    private val RARITY4 = ResourceLocation("sharttils", "gui/rarity4.png")
    private val CUSTOMRARITY = ResourceLocation("sharttils", "gui/customrarity.png")
    ...
    fun renderRarity(itemStack: ItemStack?, xPos: Int, yPos: Int) {
        if (itemStack != null) {
            if (!Sharttils.config.showPetRarity && ItemUtil.isPet(itemStack)) {
                return
            } else {
                if (!mutex.isLocked) {
                    mutex.lock()
    ...
    fun renderRarity(xPos: Int, yPos: Int, rarity: ItemRarity) {
        val alpha = Sharttils.config.itemRarityOpacity
        ...
        mc.textureManager.bindTexture(
            when (Sharttils.config.itemRarityShape) {
                0 -> RARITY
                1 -> RARITY2
                2 -> RARITY3
                3 -> RARITY4
    ...
    private fun renderRarity(xPos: Int, yPos: Int, itemStack: ItemStack?) {
        ...
        if (rarity != ItemRarity.NONE) {
            ...
            if (Sharttils.config.itemRarityShape < 5) {
