...
class ComboToast(input: String) : IToast<ComboToast> {
    ...
    private var buffTexture: ResourceLocation? = ResourceLocation("sharttils:combo/comboFail.png")
        ...
        if (comboMatcher.find()) {
            ...
            when {
                input.contains("Magic Find") -> {
                    buffTexture = ResourceLocation("sharttils:toasts/combo/luck.png")
                }
                input.contains("coins per kill") -> {
                    buffTexture = ResourceLocation("sharttils:toasts/combo/coin.png")
                }
                input.contains("Combat Exp") -> {
                    buffTexture = ResourceLocation("sharttils:toasts/combo/combat.png")
                }
            }
        } else {
            buffTexture = null
