...
fun renderCustomHeadLayer(
    ...
    if (entity is EntityPlayerSP) {
        if (Sharttils.config.transparentHeadLayer == 0f) {
            GlStateManager.popMatrix()
            ci.cancel()
            return
        }
        if (Sharttils.config.transparentHeadLayer != 1f) {
            ...
            GlStateManager.color(1f, 1f, 1f, Sharttils.config.transparentHeadLayer)
        }
    }
}
...
fun renderGlintOnSkull(
    ...
    if (Utils.inSkyblock && Sharttils.config.enchantGlintFix && itemStack.hasEffect()) {
