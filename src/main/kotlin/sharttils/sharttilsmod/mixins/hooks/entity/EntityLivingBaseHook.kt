...
import sharttils.sharttilsmod.features.impl.dungeons.WitherKingDragons
...
class EntityLivingBaseHook(val entity: EntityLivingBase) {
    ...
    fun onNewDisplayName(s: String) {
        ...
        if (Sharttils.config.commaDamage && entity is EntityArmorStand && entity.ticksExisted < 300 && s.isNotEmpty()) {
    ...
    fun modifyPotionActive(potionId: Int, cir: CallbackInfoReturnable<Boolean>) {
        if (!Utils.inSkyblock) return
        if (Sharttils.config.disableNightVision && potionId == Potion.nightVision.id && entity is EntityPlayerSP) {
    ...
    fun removeDeathParticle(
        ...
        return !Sharttils.config.hideDeathParticles || !Utils.inSkyblock || particleType != EnumParticleTypes.EXPLOSION_NORMAL
    }
