...
import sharttils.sharttilsmod.features.impl.misc.SummonSkins.skintextureMap
import sharttils.sharttilsmod.utils.SuperSecretSettings
import sharttils.sharttilsmod.utils.Utils
import sharttils.sharttilsmod.utils.printDevMessage
...
class AbstractClientPlayerHook(player: Any) {
    ...
    val summonType: Future<String?> by lazy {
        ...
        Sharttils.threadPool.submit(::findTypeEntity)
    }
    ...
    fun replaceSkin(cir: CallbackInfoReturnable<ResourceLocation>) {
        if (isSummonMob) {
            if (Sharttils.config.fixSummonSkin) {
                if (correctSkin == null) {
                    ...
                    correctSkin = skintextureMap[summonType.get()]?.resource
                        ?: (if (Sharttils.config.usePlayerSkin || SuperSecretSettings.noSychic) mc.thePlayer.locationSkin else sychicSkin).also {
                            UChat.chat(
                                "§cPlease tell a Sharttils dev to add a skin for ${summonType.get()}"
                            )
                        }
                ...
                cir.setReturnValue(if (Sharttils.config.usePlayerSkin || SuperSecretSettings.noSychic) mc.thePlayer.locationSkin else sychicSkin)
            }
        }
    }
    ...
    fun replaceSkinType(cir: CallbackInfoReturnable<String>) {
        if (isSummonMob) cir.returnValue =
            if (Sharttils.config.usePlayerSkin || SuperSecretSettings.noSychic) mc.thePlayer.skinType else "slim"
    }

    companion object {
        val sychicSkin = ResourceLocation("sharttils:sychicskin.png")
